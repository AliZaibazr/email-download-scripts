...
(async () => {
    ...
    for (const email of getEmails()) {
      ...
      await page.waitForSelector('#imapuser', {timeout: 0})
    }
...
async function Login(page,cursor,var_email,var_password) {
 ...
 await page.waitForSelector('#btnLogin', {timeout: 0})
 await cursor.move("#btnLogin");  
 await cursor.click();  
